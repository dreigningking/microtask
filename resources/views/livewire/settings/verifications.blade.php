<div class="bg-white rounded-lg shadow-md p-6 dark:bg-gray-800">
    ...
    @forelse($verificationRequirements as $category => $req)
        <div class="mb-6">
            ...
            <div class="space-y-4">
                @foreach($req['docs'] as $docType)
                    @php
                        $verification = $userVerifications[$docType] ?? null;
...
<div>
                                <h5 class="font-medium text-gray-800 dark:text-white">{{ \Illuminate\Support\Str::title(str_replace('_', ' ', $docType)) }}</h5>
                            ...
                            @if($status !== 'approved')
                                <div class="mt-4 md:mt-0">
                                    <form wire:submit.prevent="saveVerification('{{ $docType }}')">
                                        <div class="flex items-center space-x-2">
                                            <input type="file" wire:model="uploads.{{ $docType }}" id="{{ $docType }}_upload" class="block w-full text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-primary/10 file:text-primary hover:file:bg-primary/20 dark:file:bg-primary/20 dark:file:text-white dark:hover:file:bg-primary/30"/>
                                            <button type="submit" class="px-3 py-2 text-sm bg-primary text-white rounded-md hover:bg-primary/90" wire:loading.attr="disabled" wire:target="uploads.{{ $docType }}">
                                                <span wire:loading.remove wire:target="uploads.{{ $docType }}">Submit</span>
                                                <span wire:loading wire:target="uploads.{{ $docType }}">Uploading...</span>
                                            </button>
                                        </div>
                                        <p class="text-xs text-gray-500 mt-1 dark:text-gray-400">Accepted file types: JPG, PNG, PDF. Max size: 2MB.</p>
                                         @error('uploads.' . $docType) <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
                                    </form>
                                </div>
                            @endif
                        </div>

                        @if ($uploads[$docType] ?? false)
                            <div class="mt-2">
                                <p class="text-sm text-gray-600 dark:text-gray-400">Preview:</p>
                                @if (str_starts_with($uploads[$docType]->getMimeType(), 'image/'))
                                    <img src="{{ $uploads[$docType]->temporaryUrl() }}" class="h-20 rounded-md mt-1">
                                @else
                                    <div class="flex items-center space-x-2 p-2 bg-gray-100 dark:bg-gray-700 rounded-md mt-1">
                                        ...
                                        <p class="text-sm text-gray-600 dark:text-gray-300">{{ $uploads[$docType]->getClientOriginalName() }}</p>
                                    </div>
                                @endif
                            </div>
