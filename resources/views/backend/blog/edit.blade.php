...
<main class="content">
    <div class="container-fluid">
        <div class="header d-flex align-items-center mb-4">
            ...
            <a href="{{ route('admin.blog.index') }}" class="btn btn-light ml-auto"><i class="ri-arrow-left-line"></i> Back to List</a>
        </div>
        <div class="card">
            <div class="card-body">
                            ...
                            <div class="card bg-light border-0 mb-4">
                                ...
                                <div class="card-body">
                                    <div class="mb-3">
                                        ...
                                        <input type="text" name="meta_title" class="form-control" value="{{ old('meta_title', $post->meta_title) }}" placeholder="Leave empty to use post title">
                                        @error('meta_title') <div class="text-danger text-xs mt-1">{{ $message }}</div> @enderror
                                    </div>
                                    <div class="mb-3">
                                        <label class="form-label">Meta Description</label>
                                        <textarea name="meta_description" class="form-control" rows="2" placeholder="Leave empty to use post excerpt">{{ old('meta_description', $post->meta_description) }}</textarea>
                        ...
                        <div class="col-md-4">
                            <div class="mb-4">
                                ...
                                @if($post->featured_image)
                                    <div class="mb-2">
                                        <img src="{{ $post->featured_image_url }}" alt="Current Image" class="img-fluid rounded shadow border" style="max-height: 180px;">
                                        <small class="text-muted d-block mt-1">Current image</small>
                                    </div>
                                @endif
                                <input type="file" name="featured_image" class="form-control" accept="image/*" onchange="previewImage(event)">
                                <small class="text-muted">Leave blank to keep current image. Recommended: 1200x420px</small>
                            ...
                            <div class="mb-4">
                                ...
                                <input type="text" name="tags" class="form-control" value="{{ old('tags', is_array($post->tags) ? implode(', ', $post->tags) : '') }}" placeholder="tag1, tag2, tag3">
                            ...
                            <div class="mb-4">
                                ...
                                <input type="datetime-local" name="published_at" class="form-control" value="{{ old('published_at', $post->published_at ? $post->published_at->format('Y-m-d\TH:i') : '') }}">
                                @error('published_at') <div class="text-danger text-xs mt-1">{{ $message }}</div> @enderror
                            </div>
                            
                            <div class="mb-4">
                                <div class="form-check">
                                    <input type="checkbox" name="featured" class="form-check-input" value="1" {{ old('featured', $post->featured) ? 'checked' : '' }}>
                            ...
                            <div class="mb-4">
                                <div class="form-check">
                                    <input type="checkbox" name="allow_comments" class="form-check-input" value="1" {{ old('allow_comments', $post->allow_comments) ? 'checked' : '' }}>
                            ...
                            <button class="btn btn-primary w-100 flex items-center justify-center gap-2" type="submit">
                                <i class="ri-save-line"></i> <span>Update Post</span>
                            </button>
                        </div>
                    </div>
                </form>
