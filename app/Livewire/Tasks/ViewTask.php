    ...
    public function submitTask()
    {
        ...
        foreach ($this->submissionFields as $field) {
            if (isset($field['required']) && $field['required']) {
                if ($field['type'] === 'file') {
                    $rules['submittedData.' . $field['name']] = 'required|file|max:10240'; // 10MB max
                } else {
                    $rules['submittedData.' . $field['name']] = 'required';
                }
            }
        }
        ...
        foreach ($this->submissionFields as $field) {
            ...
            if ($field['type'] === 'file') {
                if ($fieldValue instanceof \Livewire\Features\SupportFileUploads\TemporaryUploadedFile) {
